...
I see that you are manually parsing files with `open()` and doing `.split` on each line. There is nothing incorrect here and your code is fine 🙂. However, we should consider using existing file parsers if already available. This (a) makes life easier as we dont need to reinvent the wheel and (b) minimises errors as we use a package that has been used and tested. I recommend rewriting the first block to use `pandas` instead. It is compatible with `scikit-learn` so there should be no problems. If `pandas` is not already installed, you can install it in colaboratory with:
