...
def rename_folder(in_folder, dst_folder):
   
    in_folder = in_folder # folder where user is uploading input
    for count, filename in enumerate(os.listdir(in_folder)):
        dst = f"chr{str(count+1)}.fa" 
        src =f"{in_folder}/{filename}"  # foldername/filename, if .py file is outside folder
        dst =f"{dst_folder}/{dst}" # folder where input is supposed to be uploaded for dna2vec to run
...
dst_folder = '/dna2vec/inputs/hg38'
rename_folder(in_folder,dst_folder)

!pip install -r /content/drive/MyDrive/Ziran_Input_Data/dna2vec/requirements.txt
!pip install Bio
...
def main():
    ...
    out_txt_filename = 'embeddings.txt'
